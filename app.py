...
openai.api_key = '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'

def transcribe_and_process_audio(audio_file):
    model = whisper.load_model("medium")
    result = model.transcribe(audio_file)
    input_prompt = result["text"]
    cer = Cerebra()
    output = cer.input_prompt(input_prompt)
    return output
...
if __name__ == '__main__':
    ...
    #my_chatgpt = ChatGPT()
    with gr.Blocks(title="Cerebra") as demo:
        gr.Markdown('''
        # Cerebra
                ''')

        with gr.Column(scale=9):
            gr.Interface(fn=transcribe_and_process_audio, inputs=audio_input, 
                    outputs=output_text, title="Access Your Second Brain",
                    description="Upload an Journal or Question Audio file and hit the 'Submit'\
                        button")
